...
- windows: sửa Ctrl+Shift+Z không ăn trên backend Hook — nguyên nhân không phải xung đột phím tắt với app mà là phím do buttre tự inject bị chính modifier người dùng đang giữ biến thành shortcut. Hotkey được poll từ vòng lặp tray mỗi 50 ms nên khi inject diễn ra, Ctrl và Shift vẫn đang bị giữ vật lý; app nhận `Ctrl+Shift+Backspace` (xoá cả từ trước trong Word và VS Code, âm thầm mất chữ thay vì thay chữ) và phần Unicode bị đẩy sang accelerator thay vì chèn vào văn bản. Đây là đường inject duy nhất trong hook có modifier đang giữ. Cách sửa là **hoãn** inject tới khi anh thả chord (vòng lặp tray đã tick mỗi 50 ms nên độ trễ không đáng kể, và đó cũng đúng lúc người dùng chờ thấy kết quả). Không nhả/nhấn lại modifier bằng phím ảo: `SendInput` cập nhật luôn async key state, nên sau khi tự nhả Ctrl thì không còn cách nào biết người dùng còn giữ hay không — nhấn lại vô điều kiện có nguy cơ kẹt phím ở trạng thái nhấn, còn chỉ nhấn lại phím "vẫn đang giữ" thì không nhấn lại gì cả, để hệ thống tưởng người dùng đã thả và **mọi lần nhấn lặp lại của chord bị nuốt** (lần đầu ăn, lần hai thành `z` trơn). Đổi chord khác không giải quyết được, miễn là chord còn chứa Ctrl hoặc Alt
