...
### Học cá nhân hóa (Personal Learning)

**Mô-đun**: `crates/buttre-core/src/state/learning.rs` + `Keyboard::collect_pending_learning()` (`crates/buttre-core/src/keyboard/engine.rs`)

Học cá nhân hóa là một cơ chế **tự động, dựa trên hành vi typing** (phân biệt với gõ tắt là user-authored — xem ADR-0001). Hệ thống hai bảng lưu trữ `learning.toml`:
- **user_attested**: âm tiết người dùng gõ trực tiếp (adjacent, không suy diễn) ≥3 lần riêng biệt — engine sau đó loại bỏ cáo hiệu "từ không xác thực" cho nó.
- **prefs**: những hành động ý thức (double-tap undo, toggle từ) trên một chuỗi phím cụ thể — lần typing tiếp theo EXACT sequence ấy sẽ áp dụng lựa chọn người dùng.

**Collection points** (per-model khác nhau):
- **Multiword mode** (Windows Hook, legacy): scroll-out-overflow tại word boundary khi từ vào queue freeze.
- **Composition mode** (TSF, IBus, Wayland-native, macOS FFI): `ConfirmComposition` action dòng chảy — composition vừa chốt từ là thời điểm thu thập.

**Many-writer merge model**: `learning.toml` được ghi bởi tray, từng ứng dụng TSF, và engine Linux process. Mỗi ghi đi qua `LearningStore::write_atomic_merged` — đọc trạng thái on-disk hiện tại trước đó, sáp nhập union thêm vào (additive chỉ — học không lỗi lạc, không bao giờ lùi), rồi atomic-rename. Xem [`learning_writer.rs`](crates/buttre-platform/src/shared/learning_writer.rs) — engine Linux + mỗi TSF host có thread ghi riêng ngoài đường phím, debounce burst snapshots xuống cái cuối cùng, merged-write.

**Toggle `Settings::learning_enabled`**: bật/tắt ngay lập tức, không cần restart, qua file-watch trên mọi backend — engine Linux + tray đều đọc lazy per-keystroke (cùng mô hình như strict/enabled).
